...
def train():
    ...
    for epoch in range(epoch_n):
        ...
        while not data_loader.is_end():
            ...
            loss.backward()  
            optimizer.step()
            net.apply_clipper()  # Monotonicity assumption

            running_loss += loss.item()
            if batch_count % 200 == 199:
                running_loss = 0.0

        rmse, auc, accuracy = validate(net, epoch)

        # Save the model if it has the best accuracy so far
        if accuracy > best_accuracy:
            best_accuracy = accuracy
            save_snapshot(net, 'model/best_model')  # Save the best model with a consistent name
            print(f"New best model found with accuracy: {accuracy:.4f}. Model saved.")

        # Optionally, save the model after every epoch
        save_snapshot(net, f'model/model_epoch_{epoch + 1}')
...
def validate(model, epoch):
    ...
    return rmse, auc, accuracy  # Return accuracy as well
